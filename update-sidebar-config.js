 ...
 * 从Notion获取侧边栏配置数据
 */
async function getSidebarConfigFromNotion() {
  try {
...
async function updateSidebarConfig() {
  try {
    ...
    // 1. 从Notion获取侧边栏配置
    const sidebarConfig = await getSidebarConfigFromNotion();
    ...
    const sidebarConfigString = `// 侧边栏
sidebar: ${JSON.stringify(sidebarConfig, null, 4)}`;
    
    // 5. 查找并替换现有的侧边栏配置
    const sidebarRegex = /\/\/\s*侧边栏\s*\n\s*sidebar:\s*{[^}]*}/;
    ...
    console.log(`🎉 成功更新侧边栏配置，添加了 ${Object.keys(sidebarConfig).length} 个分类`);
    return { success: true, message: `更新了 ${Object.keys(sidebarConfig).length} 个分类` };
  } catch (error) {
...
export async function runUpdate(options = {}) {
    ...
    const result = await updateSidebarConfig();
    if (result.success) {
      console.log('🎉 侧边栏配置更新完成！');
    } else {
